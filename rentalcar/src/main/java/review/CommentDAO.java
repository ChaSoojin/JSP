...
public class CommentDAO {
	...
	public boolean addComment(String id, int no, String comment) {
		try {
			...
			String str = "insert into review_comment(id,reviewno,comment,regdate) values(?, ?, ?, ?)";
	...
	public ArrayList<CommentDTO> getComments(int no) {
		try {
			conn = DBManager.getConnection();
			String str = "select * from review_comment where reviewno = '" + no + "'";
			pstmt = conn.prepareStatement(str);
			...
			while (rs.next()) {
				...
				int reviewno = rs.getInt(3);
				String comment = rs.getString(4);
				int likes = rs.getInt(5);
				int notlikes = rs.getInt(6);
				Timestamp regDate = rs.getTimestamp(7);

				comments.add(new CommentDTO(cnt, originalno, id, reviewno, comment, likes, notlikes, regDate));
			}
	...
	public boolean deleteComment(int no, int originalno) {
		...
		try {
			conn = DBManager.getConnection();
			String str = "delete from review_comment where no = '" + originalno + "' and reviewno = '" + no +"'";
			pstmt = conn.prepareStatement(str);
			pstmt.executeUpdate();
	...
	//댓글 추천 비추천
	public boolean updateCommentLikes(int chk, int reviewno, int commentno) {
		try {
			int idx = -1;
			comments = getComments(reviewno);
			...
			if(chk == 1) {
				...
				str = "update review_comment set likes = ? where reviewno = '" + reviewno + "' and no = '" + commentno + "'";				
			}
			...
			//비추천
				...
				str = "update review_comment set notlikes = ? where reviewno = '" + reviewno + "' and no = '" + commentno + "'";								
			}
